...
import {ILAWPYieldVault} from "../../src/interfaces/ILAWPYieldVault.sol";
...
contract LAWPYieldVaultTest is Test {
    ...
    function setUp() public {
        token = new MockCNGN();
        vault = new LAWPYieldVault(address(token));
    ...
    function test_RevertIf_ConstructorZeroAddressToken() public {
        ...
        new LAWPYieldVault(address(0));
    }

    function test_RevertIf_SetComplianceEngineZeroAddress() public {
        LAWPYieldVault newVault = new LAWPYieldVault(address(token));
        vm.expectRevert(LAWPYieldVault.LAWPYieldVault_InvalidAddress.selector);
        newVault.setComplianceEngine(address(0));
    }

    function test_RevertIf_AlreadyInitialized() public {
        vm.expectRevert(LAWPYieldVault.LAWPYieldVault_AlreadyInitialized.selector);
    ...
    function test_ExecuteTransfer_Success() public {
        ...
        emit YieldTransferExecuted(receiver, amount);
